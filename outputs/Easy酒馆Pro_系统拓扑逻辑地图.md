...
> 生成日期: 2026-07-05 | 版本: v2.0（全量重写 · 基于源码实读）
...
| 文件 | 职责 |
|------|------|
| `utils/constants.ts` | 全局常量、默认配置、11 个 `DEFAULT_TPL_*` 模板、气泡颜色/对齐/标签映射 |
...
| 文件 | 职责 |
|------|------|
| `useApp.tsx` | 全局状态 reducer + Context + IndexedDB 持久化（24 个 action） |
| `useChat.ts` | 消息发送/旁听/蒸馏触发/状态书/Galgame 自动触发/SSE 流式（30s 空闲超时） |
...
| `useConversations.ts` | 对话 CRUD + best-effort 删除清理 |
| `useCharacters.ts` | 角色 CRUD |
| `useWorldBooks.ts` | 世界书及条目 CRUD + 批量导入 |

### db/
| 文件 | 职责 |
|------|------|
| `db/index.ts` | 7 个 localForage store 实例 + `initDB()` |
...
| `ui/` | Button, Toggle, Modal, Dropdown, Icon（17 个 SVG）, Tooltip |
| `characters/` | CharacterManager（头像压缩 + SillyTavern 导入导出）, CharacterSelector |
| `conversations/` | ConversationList（新建/删除/导出 TXT，navigator.share 优先） |
| `models/` | ModelManager（采样三预设 + Ping）, ModelPing |
| `worldbook/` | WorldBookManager（AI 指南 prompt + 条目 CRUD + JSON 批量导入导出） |
...
```
     ...
     └──────────┬───────────────────┘
                ▼
     ┌──────────────────────────────┐
     │ 1. 世界书扫描 · 关键词匹配    │  ← escapeRegExp 防ReDoS + includes快路径
...
| 1 | 世界书扫描 | `escapeRegExp` 防 ReDoS；`includes` 快路径 → RegExp 兜底；priority 降序；逐条独立冷却 |
...
| 上下文 | `[system(prompt), user(纯对话)]` 两段式 | `[system, user(角色卡), assistant(状态书×N), user(对话)]` 四段式 |
| 输入范围 | 全部非系统消息 | 最近 2 轮（`cleanDialogueText` 清洗） |
| Token / 温度 | 模型默认 | max_tokens=500, temperature=0.3, top_p=0.85 |
...
四段式上下文组装:
  ...
  ↓
参数: max_tokens=500, temperature=0.3, top_p=0.85
  ↓
...
cleanObservation 后处理:
  ...
  ↓
写入世界书:
  为每个角色在其世界书插入/覆盖 key=对方角色名、priority=10 的条目
  若角色无世界书或绑定已被删除 → 自动新建并绑定
```
...
| 导入数据 | JSON 恢复，**强制清空 apiKey**（需手动重填） |
| 存储 | 全部 IndexedDB，不上传 |

### Section 7 — ⚠️ 高级提示词设置（可折叠）

警告框（theme-aware `bg-red-50 dark:bg-red-950/30`），展开后 11 个模板编辑器：
...
SET_SCRIBE_ENABLED, SET_SCRIBE_INTERVAL, SET_SCRIBE_TRIGGER_INTERVAL,
...
| 存储层 | Store | 内容 | 范围 |
|--------|-------|------|------|
| 持久化 UI | `ui_settings` | theme, wallpaper, boldColorize, scribeEngine/Mode, galgamePrompt, mutualObservePrompt, 11 个 tpl*, thinkingEnabled, debugMode, scribeEnabled | 跨会话全局 |
| 会话级 | `global_states` | scribeModelId, scribeEnabled, scribeTriggerInterval, scribeSystemPrompt | 每对话独立 |
...
| `tavern_characters` | 角色卡 | getAllCharacters / getCharacterById / addCharacter / updateCharacter / deleteCharacter |
...
┌──────────────────────────────┴──────────────────────────────────────┐
│  ② 功能子系统                                                        │
│  世界书扫描器(escapeRegExp防ReDoS · 逐条冷却)                          │
│  状态书/Galgame双引擎(text总结 / galgame四段式非对称数值)              │
...
┌──────────────────────────────┴──────────────────────────────────────┐
│  ③ 核心引擎层                                                        │
│  上下文组装引擎(assembleContext · Token预算 · 角色隔离 · 11模板可编辑)  │
...
┌──────────────────────────────┴──────────────────────────────────────┐
│  ④ 状态与配置层                                                      │
│  useApp reducer(24 actions) · 24个APPState字段持久化                   │
│  高级提示词11模板(SET_ADV_TPL) · boldColorize · theme/wallpaper       │
│  互相认识Prompt · GalgamePrompt · 蒸馏配置 · 上下文配置                  │
└──────────────────────────────┬──────────────────────────────────────┘
                               │
┌──────────────────────────────┴──────────────────────────────────────┐
│  ⑤ 数据持久化层 · IndexedDB (localForage) + 写锁                      │
│  models · characters · conversations · message_nodes                  │
│  worldbooks · global_states · ui_settings                             │
...
```
角色卡(systemPrompt, 名字, 头像)
     ├──→ assembleContext (System Prompt 注入)
     │
世界书词条(keywords, value, priority)
     ├──→ useWorldBookScanner (关键词匹配 + escapeRegExp + 逐条冷却)
...
| 高级提示词不可编辑 | 10 项硬编码用户无法审计 | 全链路 11 模板可编辑 + SettingsPanel 折叠编辑器 |

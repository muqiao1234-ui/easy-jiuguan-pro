...
> 更新日期: 2026-07-19 | 版本: v2.4（完整轮次蒸馏 · 并发提交保护 · 可编辑记忆回廊）
...
- **缓存世界书限制**：仅显式 `kind='cache'` 的世界书可由缓存书 UI/AI 修改；写入最终只保留 10 条，避免同为 10 条的手动世界书被误判和截断
...
```
触发: 手动 triggerDistillation / 自动；二者共用 planDistillation
  ↓
按 user 起点划分完整轮次，只选择最旧 threshold 轮，并保留最近 retainRecentCount 轮
  ↓
拼装选中轮次文本 "{senderName}: {content}\n\n"
  ↓
...
生成 distilled 节点:
  ...
  内容 = tplDistilledNodePrefix 模板（📝 记忆 第{start}轮-第{end}轮:{summary}）
  timestamp = 最后被蒸馏节点 ts + 1ms  ← 防蒸馏期间新消息导致时序错乱
  distillationMeta = 来源节点 ID + 真实轮次范围 + cumulative=true
  ↓
commitDistillationBatch 在消息 store 写锁内复核来源 → 归档原节点 + 写入结晶；失败回滚
```

- **并发保护**：执行器使用同步 `inFlightRef`；存储提交再次校验来源节点未归档，自动/手动重叠不会重复写入
- **累计记忆**：新结晶合并上一份累计记忆；上下文只注入最新累计结晶，旧结晶保留在可编辑的记忆回廊
...
| 设置项 | 控件 | Action | 说明 |
|--------|------|--------|------|
| 最少完整对话轮数 | range 5-50 | `UPDATE_DISTILLATION_CONFIG {triggerThreshold}` | 可蒸馏完整轮次达到阈值才执行 |
| 保留最近轮数 | range 0-15 | `UPDATE_DISTILLATION_CONFIG {retainRecentCount}` | 最近 N 个完整轮次不参与蒸馏 |
...
| `tavern_message_nodes` | v3 每节点分片 + 每会话元数据索引（timestamp/id/role/archive/scribe） | getMessageNodesPageByConversation / queryMessageNodesByConversation / countMessageNodesByConversation / getMessageNodesByConversation / addMessageNode(s) / updateMessageNode(s) / deleteMessageNode(s) |
...
| 蒸馏单轮/半轮 | 手动把阈值当最大值，自动按消息条数硬切 | 自动/手动共用完整轮次规划器；不足阈值不执行；最近 N 轮整体保留 |
| 蒸馏重复提交 | 自动与手动可能同时选择同一批节点 | `inFlightRef` 单任务互斥 + `commitDistillationBatch` 提交时复核与回滚 |
| 空摘要归档原文 | 模型 content 为空仍写“蒸馏失败”并归档 | 空摘要直接报错；原节点不归档；兼容 reasoning_content 回退 |
...
| 缓存世界书误伤 | 名称或 10 条上限启发式可能把手动世界书改成缓存书 | 仅接受显式 `kind='cache'`；AI 写入前二次校验类型；JSON operation 逐项校验并阻止重复写入 A 世界书关键词 |
| 长对话发送卡顿 | UI 虽分页，但发送/状态书/蒸馏仍逐条读取完整历史 | v3 复合游标分页 + 索引有限查询/计数；常规发送只取最近窗口，蒸馏只取最旧批次 |
| GitHub Pages 黑屏 | 工作流直接发布源码，浏览器无法执行 `/src/main.tsx` | Actions 先执行 `npm ci && npm run build`，仅发布 `dist/`；Vite 使用相对 base |

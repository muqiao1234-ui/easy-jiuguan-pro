...
🧭 第一部分：世界书的底层超能力是什么？
如果把大量小说设定、配角资料和术语全部塞进角色人设（System Prompt），会增加上下文长度，也可能降低 AI 对当前剧情的关注。
...
未匹配时不注入：如果当前对话没有提到这些关键词，对应条目会留在本地，不会进入本轮上下文。
...
⚠️ 注意：核心 AI 的性格、外貌和关系设定，请优先写在【角色卡人设】里。

⚡ 第二部分：使用外部 AI 辅助整理世界书
如果不想逐条填写 Key 和 Value，可以使用外部 AI 按模板整理世界观资料，再导入 Easy酒馆Pro。
...
第二步：调用外部 AI 生成数据
打开你常用的联网 AI，将命令里的 [请输入你想查询的作品/设定，例如：魔戒] 替换成目标背景后发送。

第三步：复制 JSON 并一键粘贴
请复制 AI 返回的 JSON 数组，回到 Easy酒馆Pro，点击【新建世界书】→【导入 JSON】，粘贴并保存。

第四步：角色卡关联绑定（超级重要，90%的新人都漏了这步！）
导入后，世界书默认不会自动绑定。请打开正在聊天的【角色卡设置】，在“世界书关联”中选择这本设定集。
绑定完成后，匹配到关键词时，相关条目才会进入对话上下文。

🎉 设置完成后，就可以导入并使用自己的世界书了。`;

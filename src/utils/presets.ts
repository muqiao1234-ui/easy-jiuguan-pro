...
import type { ModelConfig, Character, Conversation, MessageNode, WorldBook, GalgameData } from '../types';
...
const CONV_0_MESSAGES: MessageNode[] = [
  ...
  msg(PRESET_CONV_IDS[0], 'charA', '（停下手中的记录，微笑着摸了摸花生的头）别急，小家伙。这个软件是个"本地数据"的宝箱，所有数据都锁在你的浏览器本地缓存里，较为安全。所以第一步，你需要给它接上"电力"和"灵魂"。'),
...
const CONV_3_MESSAGES: MessageNode[] = [
  ...
  msg(
    PRESET_CONV_IDS[3],
    'charA',
    '（推眼镜，耳朵微微抖动）……记录，测试对象花生试图篡改数值。不过没关系，这正是状态书手动编辑的用途——你可以随时修正数值，也可以重置回初始值。记住，状态书是"对话的活档案"，它让长对话里的状态不再飘忽不定。',
    {
      name: '核桃',
      health: 'fine',
      mood: 'shy',
...
const CONV_5_MESSAGES: MessageNode[] = [
  ...
  msg(PRESET_CONV_IDS[5], 'charA', '（满意地点头）很好。记住，结构化角色卡是"灵魂小抄"，别名魔术是"记忆保险"，采样参数是"性格调节旋钮"——这三件套是高级玩家的角色加固武器。'),
  msg(PRESET_CONV_IDS[5], 'charB', '（站起来，向屏幕前的玩家鞠躬）屏幕前的小伙伴，五天的教学到这里就结束啦！从配 API 到调参数，你现在已经是 Easy酒馆Pro 的高手了！快去创造你自己的故事吧，吱！'),
  msg(PRESET_CONV_IDS[5], 'charA', '（合上记录板，微笑）记录，教学周期结束。祝你在 Easy酒馆Pro 里玩得开心。如果你忘了任何操作，随时回来翻阅这套预设对话——我们一直在这里。'),
];
...
export async function seedPresets(): Promise<void> {
  try {
    ...
    console.log('[seedPresets] 预设资源检查完毕：1 模型 + 2 角色 + 1 世界书 + 6 对话');
  } catch (e) {
